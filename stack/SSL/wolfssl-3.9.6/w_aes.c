/* aes.c (w_aes.c for uTasker project use)
 *
...
#define HAVE_CONFIG_H                                                    // {1}
    ...
    #include "config.h"
...
#if defined CRYPTO_WOLF_SSL && defined CRYPTOGRAPHY && defined CRYPTO_AES// {1}
    #if !defined _WINDOWS && (defined CAU_V1_AVAILABLE || defined CAU_V2_AVAILABLE) && !defined AES_DISABLE_CAU // if the processor being used has CAU and not disabled
        #define FREESCALE_MMCAU                                          // enable wolfSSL usage
        ...
        static int wolfSSL_CryptHwMutexLock(void) { return 0; }
        static int wolfSSL_CryptHwMutexUnLock(void) { return 0; }
    #endif
...
//#include "logging.h"                                                   {1}
    ...
    #include "misc_code.h"                                               // {1} changed to header file to avoid confusing ecipse based environments
#endif
#ifdef DEBUG_AESNI
    #include <stdio.h>
#endif
...
/* Define AES implementation includes and functions */
  ...
  //#include "cau_api.h"                                                 // {1}
    ...
    static int wc_AesEncrypt(Aes* aes, const byte* inBlock, byte* outBlock)
    {
        ...
        if(ret == 0) {
            mmcau_aes_encrypt(inBlock, (byte*)aes->key, aes->rounds, outBlock);
    ...
    static int wc_AesDecrypt(Aes* aes, const byte* inBlock, byte* outBlock)
    {
        int ret = wolfSSL_CryptHwMutexLock();
        if(ret == 0) {
            mmcau_aes_decrypt(inBlock, (byte*)aes->key, aes->rounds, outBlock);
            wolfSSL_CryptHwMutexUnLock();
        }
        return ret;
    }
...
static void wc_AesEncrypt(Aes* aes, const byte* inBlock, byte* outBlock)
{
  ...
  //if (r > 7 || r == 0) {
  //    WOLFSSL_MSG("AesEncrypt encountered improper key, set it up");   // {1} checked before calling
...
static void wc_AesDecrypt(Aes* aes, const byte* inBlock, byte* outBlock)
{
    ...
    const word32* rk = aes->key;
  //if (r > 7 || r == 0) {
  //    WOLFSSL_MSG("AesDecrypt encountered improper key, set it up");   // {1} checked before calling
  //    return;  /* stop instead of segfaulting, set up your keys! */
  //}
#ifdef WOLFSSL_AESNI
    if (haveAESNI && aes->use_aesni) {
...
/* wc_AesSetKey */
                  ...
                  int dir)
    {
        ...
        if(ret == 0) {
            mmcau_aes_set_key(userKey, keylen*8, rk);
            wolfSSL_CryptHwMutexUnLock();
...
/* AES-CBC */
    ...
    int wc_AesCbcEncrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
      ...
      //if ((wolfssl_word)out % WOLFSSL_MMCAU_ALIGNMENT) {               // {1} ensured by calling routine
