/**********************************************************************
    ...
    *********************************************************************
    ...
    21.06.2019 Allow slave callbacks to reject message reception in order to cause/test no response timeouts at the master {7}
    25.08.2019 Add fnGetModusSocketCtl()                                 {V1.31}
...
#if MODBUS_TCP_SERVERS > 0 && defined USE_MODBUS_SLAVE
static unsigned char fnGetTCP_session_count(unsigned char ucMODBUSport, unsigned char *ptrOffset)
{
    unsigned char ucTCP_slave_port = (ucMODBUSport - MODBUS_SERIAL_INTERFACES);
    unsigned char ucTotalCnt;
    unsigned char ucSocketCount;
    switch (ucTCP_slave_port) {
#if defined MODBUS_SOCKETS_1
    case 1:
        ucSocketCount = MODBUS_SOCKETS_1;
        ucTotalCnt = MODBUS_SOCKETS_0;
        break;
#endif
#if defined MODBUS_SOCKETS_2
    case 2:
        ucSocketCount = MODBUS_SOCKETS_2;
        ucTotalCnt = (MODBUS_SOCKETS_0 + MODBUS_SOCKETS_1);
        break;
#endif
#if defined MODBUS_SOCKETS_3
    case 3:
        ucSocketCount = MODBUS_SOCKETS_3;
        ucTotalCnt = (MODBUS_SOCKETS_0 + MODBUS_SOCKETS_1 + MODBUS_SOCKETS_2);
        break;
#endif
#if defined MODBUS_SOCKETS_4
    case 4:
        ucSocketCount = MODBUS_SOCKETS_4;
        ucTotalCnt = (MODBUS_SOCKETS_0 + MODBUS_SOCKETS_1 + MODBUS_SOCKETS_2 + MODBUS_SOCKETS_3);
        break;
#endif
    default:
        ucSocketCount = MODBUS_SOCKETS_0;
        ucTotalCnt = 0;
        break;
    }
    if (ptrOffset != 0) {
        *ptrOffset = ucTotalCnt;
    }
    return ucSocketCount;
}
#endif
...
extern int fnInitialiseMODBUS_port(unsigned char ucMODBUSport, const MODBUS_CONFIG *ptrMODBUS_config, const MODBUS_CALLBACKS *ptrModbus_callbacks, int (*master_callback)(int iType, MODBUS_RX_FUNCTION *))
{
    ...
    #if MODBUS_TCP_SERVERS > 0 && defined USE_MODBUS_SLAVE
        unsigned char ucSessions = fnGetTCP_session_count(ucMODBUSport, 0);
        #if defined TCP_SLAVE_ROUTER
        fnMasterCallback[ucMODBUSport] = master_callback;
        #endif
...
static QUEUE_HANDLE fnOpenSerialInterface(unsigned char ucMODBUSport)
{
    ...
    #endif
    tInterfaceParameters.Config = ptrMODBUS_pars->SerialMode[ucMODBUSport];
    if ((ptrMODBUS_pars->ucModbusSerialPortMode[ucMODBUSport] & MODBUS_MODE_ASCII) != 0) {
    #if defined MODBUS_ASCII
...
static void fnConfigureInterspaceTimes(unsigned char ucMODBUSport)
{
    ...
    _TIMER_INTERRUPT_SETUP *ptrSetup3_5 = &timer_setup_3_5[ucMODBUSport];
        #if (defined PITS_AVAILABLE && (PITS_AVAILABLE > 0)) || (defined LPITS_AVAILABLE && (LPITS_AVAILABLE > 0))  // use PIT timers (up to 4 possible)
    ...
    if (ucMODBUSport == 1) {
        ...
        ptrSetup3_5->int_handler = fnTimer_T_3_5_fired_3;
            ...
            #endif
    switch (ptrMODBUS_pars->ucSerialSpeed[ucMODBUSport]) {
            #if defined _LM3SXXXX || defined _STM32 || defined _KINETIS || (defined _HW_TIMER_MODE && (defined _LPC23XX || defined _LPC17XX)) // {V1.10}
    case SERIAL_BAUD_1200:
...
_handle_local_slave:
    ...
    if (ucExceptionCode != 0) {
        if (MODBUS_IGNORE_RECEPTION == ucExceptionCode) {                // {7}
            return -1;                                                   // do not return a response to the master
        }
    #if defined MODBUS_SUPPORT_SERIAL_LINE_FUNCTIONS && defined MODBUS_SUPPORT_SERIAL_LINE_DIAGNOSTICS
...
extern MODBUS_TCP_CTL *fnGetModusSocketCtl(unsigned char ucModbusPort)   // {V1.31}
{
    MODBUS_TCP_CTL *ptrCtl = ptrMODBUS_TCP;
    unsigned char ucOffset = 0;
    if (ucModbusPort < MODBUS_SERIAL_INTERFACES) {
        return 0;                                                        // not a modbus tcp port
    }
    if (ptrCtl == 0) {
        return 0;                                                        // not ready
    }
    fnGetTCP_session_count(ucModbusPort, &ucOffset);
    return (ptrMODBUS_TCP + ucOffset);
}
        ...
        if (_TCP_SOCKET_MASK(modbus_session->OwnerTCPSocket) == _TCP_SOCKET_MASK(Socket)) { // {V1.23} search for the session to handle this event
            ...
            case TCP_EVENT_CONREQ:                                       // session requested
                if (modbus_session->ucState == MODBUS_STATE_FREE) {
                    ...
                    modbus_session->OwnerTCPSocket = Socket;             // {V1.23} add network, VLAN and interface details of this connection
                }
                break;
