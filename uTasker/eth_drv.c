...
#if defined _HW_AVR32 || defined _HW_SAM7X || defined _LM3SXXXX
...
static QUEUE_TRANSFER entry_eth(QUEUE_HANDLE channel, unsigned char *ptBuffer, QUEUE_TRANSFER Counter, unsigned char ucCallType, QUEUE_HANDLE DriverID)
{
        ...
        if (ptBuffer != 0) {                                             // data being passed
...
#endif
            {
                if (ptTTYQue->ETH_queue.chars == 0) {                    // first part of a frame
                    ...
                    uMask_Interrupt(EMAC_PREEMPT_LEVEL << __NVIC_PRIORITY_SHIFT); // {10} set interrupt mask level to block low priority pre-emption until the frame has been completed and released
    #endif
    #if !defined ETH_INTERFACE || !defined ETHERNET_AVAILABLE || defined NO_INTERNAL_ETHERNET || (ETHERNET_INTERFACES > 1) || defined USB_CDC_RNDIS || defined USB_RNDIS_HOST || defined USE_PPP
    ...
    #if !defined ETH_INTERFACE || !defined ETH_INTERFACE || !defined ETHERNET_AVAILABLE || defined NO_INTERNAL_ETHERNET || (ETHERNET_INTERFACES > 1) || defined USB_CDC_RNDIS || defined USB_RNDIS_HOST || defined USE_PPP
                    ...
                    {
    #if defined EMAC_PREEMPT_LEVEL
                        uMask_Interrupt(LOWEST_PRIORITY_PREEMPT_LEVEL << __NVIC_PRIORITY_SHIFT); // {10} allow pre-emption again
    #endif
