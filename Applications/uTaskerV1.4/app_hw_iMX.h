...
//
  ...
  //#define PLL2_SPREAD_SPECTRUM                                         // if set, 528MHz PLL2 will operate in spread spectrum mode with the following parameters
        #define PLL2_SPREAD_SPECTRUM_RANGE      64000                    // spread spectrum range in Hz (1..65534Hz)
        #define PLL2_SPREAD_SPECTRUM_MOD_FREQ   1000                     // spread spectrum modulation frequency in Hz (1..32767Hz)
  ...
  //#define PLL6_CLOCK_OSC_CLK                                           // keep PLL6 disabled in bypass mode and use OSC_CLK instead (else run at fixed 500MHz when required)
    ...
    // It is possible to adjust the system PLL2 between 528MHz and 552MHz (disable to use the fixed 528MHz frequency, or set to 528000000)
    //
  ...
  //#define LPSPI_CLK_FROM_PLL3_PDF1
  //#define LPSPI_CLK_FROM_PLL2_PDF2
        #define LPSPI_CLK_ROOT_DIVIDE           5//4                        // 1..8 with optional divider (valid for all sources)
  ...
  //#define USDHC2_CLK_SOURCE_PLL2_PFD0                                  // USDHC2_CLK_ROOT sourced from pll2.pfd0
    ...
    #define USB_CLOCK_GENERATED_INTERNALLY                               // use USB clock from internal source rather than external pin
#elif defined MIMXRT1050 || defined MIMXRT1060 || defined iMX_RT1052_EMB_ART || defined iMX_RT1062_EMB_ART || defined MIMXRT1064 || defined TEENSY_4_0 || defined TEENSY_4_1 || defined ARCH_MIX || defined MIMXRT1170
    #define CRYSTAL_FREQUENCY    24000000                                // 24 MHz crystal (only this crystal frequency should be used)
    #define _EXTERNAL_CLOCK      CRYSTAL_FREQUENCY                       // the clock source if the crystal oscillator
  ...
  //#define PERIPH_CLK_SOURCE_OSC                                        // PERIPHCLK sourced from OSC_CLK (24MHz) - this option is equivalent to RUN_FROM_DEFAULT_CLOCK but with additional pre-scaler option
    ...
    #define PERIPH_CLK_SOURCE_ARM_PLL                                    // PERIPHCLK sourced from ARM PLL (650MHz .. 1.3GHz)
        #if (defined iMX_RT1062_EMB_ART)                                 // industrial grade parts limited to 528MHz
  ...
  //#define FLEXSPI_CLK_FROM_PLL3_SW_CLK
    #define FLEXSPI_CLK_FROM_SEMC_CLK
        #define FLEXSPI_CLK_ROOT_DIVIDE         4//1                     // 1..8 with optional divider (valid for all sources)
  ...
  //#define LPSPI_CLK_FROM_PLL2_PDF2
    ...
    #define PERCLK_CLK_ROOT_SOURCE_IPG_CLK                               // PERCLK sourced from IPG_CLK_ROOT (as used by ADC and XBAR)
  //#define PERCLK_CLK_ROOT_SOURCE_OSC_CLK                               // PERCLK sourced from 24.0MHz OSC_CLK (used by ADC and XBAR)
        #define PERCLK_CLK_ROOT_DIVIDE          2                        // 1..64 with optional divider (valid for either IPG or OSC source)
  ...
  //#define USDHC2_CLK_SOURCE_PLL2_PFD0                                  // USDHC2_CLK_ROOT sourced from pll2.pfd0
    ...
    //
  //#define LCDIF_CLK_SOURCE_PLL2                                        // LCDIF_CLK_ROOT sourced from pll2
    //#define LCDIF_CLK_SOURCE_PLL3_PFD3                                 // LCDIF_CLK_ROOT sourced from pll3.pfd3
      #define LCDIF_CLK_SOURCE_PLL5                                      // LCDIF_CLK_ROOT sourced from pll5 (video PLL frequency = 24MHz * (VIDEO_PLL_LOOP_DIVIDER + (VIDEO_PLL_LOOP_NUMERATOR/VIDEO_PLL_LOOP_DENOMINATOR))
          #define VIDEO_PLL_LOOP_DIVIDER        30                       // possible range 27..54 [648MHz..1.296MHz]
          #define VIDEO_PLL_LOOP_NUMERATOR      0                        // if not set to 0 make sure that the numerator is smaller than the denominator (range 0..0x3fffffff)
          ...
          #else
              #define PLL5_POST_DIVIDE          4                        // 1, 2, 4, 8 or 16
          #endif
    //#define LCDIF_CLK_SOURCE_PLL2_PFD0                                 // LCDIF_CLK_ROOT sourced from pll2.pfd0
    //#define LCDIF_CLK_SOURCE_PLL2_PFD1                                 // LCDIF_CLK_ROOT sourced from pll2.pfd1
    //#define LCDIF_CLK_SOURCE_PLL3_PFD1                                 // LCDIF_CLK_ROOT sourced from pll3.pfd1

    #define LDCIF_CLK_PRESCALER                 4                        // 1..8
    #define LCDIF_CLK_SOURCE_DIVIDE             3                        // 1..8

    #define LCD_READ_PRIORITY                   3                        // 1..15 (15 is highest priority) - increase if flicker or a shft is encountered (default 1 should be adequate for smaller displays and when the buffer is in internal RAM but larger displays with SDRAM frame buffer may need an increase)
#endif
...
//
  ...
  //#define PIN_COUNT           PIN_COUNT_100_PIN                        // 100 pin LQFP package
    ...
    #define FLASH_START_ADDRESS FLEXSPI_FLASH_BASE
    #define MAXIMUM_ERASE_CHUNK_WITHOUT_WDOG_RETRIGGER  (64 * 1024)      // a 64k block erase takes typically 150ms but with a max. of 1s so we ensure watchdog retriggers between each
  ...
  //#define FLEXSPI_FLASH_SIZE  (8 * 1024 * 1024)                        // 8 Mbytes IS25LP064A-JBLE on board
    ...
    #define FLASH_START_ADDRESS FLEXSPI_FLASH_BASE
    #define MAXIMUM_ERASE_CHUNK_WITHOUT_WDOG_RETRIGGER  (256 * 1024)     // a 256k block erase takes typically 930ms but with a max. of 2.9s so we ensure watchdog retriggers between each
    #define SDRAM_SIZE          (32 * 1024 * 1024)                       // 32MByte IS42S16160J-6BLI
#elif defined MIMXRT1060 || defined MIMXRT1170
    #define PIN_COUNT           PIN_COUNT_196_PIN                        // 196 pin MAPBGA package
    #define PACKAGE_TYPE        PACKAGE_MAPBGA                           // MAPBGA
  //#define BOOT_HYPER_FLASH                                             // boot from hyper flash rather than QSPI flash
    #define FLEXSPI_FLASH_SIZE  (8 * 1024 * 1024)                        // 8 Mbytes IS25LP064A-JBLE on board
  //#define FLEXSPI_FLASH_SIZE  (64 * 1024 * 1024)                       // 64 Mbytes S26KS512SDPBHI02 on board
    ...
    #define PACKAGE_TYPE        PACKAGE_MAPBGA                           // MAPBGA
        ...
        #define FLEXSPI_FLASH_SIZE  (8 * 1024 * 1024)                    // 8 Mbytes W25Q64 on board
    #else
        #define FLEXSPI_FLASH_SIZE  (2 * 1024 * 1024)                    // 2 Mbytes W25Q16 on board
    #endif
    #define FLASH_START_ADDRESS FLEXSPI_FLASH_BASE
...
#define STOP_MII_CLOCK                                                   // only apply MII clock when needed (don't use when DEVELOP_PHY_CONTROL is set in debug.c)
...
#define SUPPORT_XBAR                                                     // support XBAR crossbar multiplexer - see XBAR and AOI video presentation at https://youtu.be/zNWIG-O7ZW0

#define SUPPORT_OCOTP                                                    // support reading from and writing to the on-chip OTP controller
  //#define MAC_FROM_USER_REG                                            // store the MAC address in the device's OTP (use together with SUPPORT_OCOTP) - WARNING: the MAC address can be programmed only once and occupies the OTP MAC eFUSE of the chip being used

#if defined MIMXRT1020 || defined MIMXRT1024 || defined MIMXRT1050 || defined MIMXRT1060 || defined iMX_RT1052_EMB_ART || defined iMX_RT1062_EMB_ART || defined MIMXRT1064 || defined MIMXRT1170
    #define MII_MANAGEMENT_CLOCK_SPEED    2500000                        // typ. 2.5MHz Speed
  //#define MSCR_HOLDTIME          7
    #define _KSZ8081RNB                                                  // the PHY used
    #if defined MIMXRT1020 && defined DEV6
        // MII interface
        //
        #define ENET0_MDIO_ON_SD
        #define RESET_PHY                  PIN_GPIO_EMC_39_GPIO3_IO07    // control the PHY's reset so that its configuration can be controlled
        #define PHY_ISOLATE                PIN_GPIO_AD_B0_12_GPIO1_IO12
        #define PHY_CONFIG0                PIN_GPIO_EMC_35_GPIO3_IO03
        #define PHY_CONFIG1                PIN_GPIO_EMC_34_GPIO3_IO02
        #define PHY_CONFIG2                PIN_GPIO_AD_B0_11_GPIO1_IO11
        #define PHY_AD0                    PIN_GPIO_EMC_37_GPIO3_IO05
        #define PHY_AD1                    PIN_GPIO_EMC_38_GPIO3_IO06
        #define PHY_AD2                    PIN_GPIO_AD_B0_09_GPIO1_IO09
        #define ASSERT_PHY_RST()           _CONFIG_DRIVE_PORT_OUTPUT_VALUE(3, (RESET_PHY | PHY_AD0 | PHY_AD1 | PHY_CONFIG0 | PHY_CONFIG1), (PHY_AD1), (PORT_SRE_SLOW | PORT_DSE_MID)) // start with PHY reset asserted
        #define CONFIG_PHY_STRAPS()        _CONFIG_DRIVE_PORT_OUTPUT_VALUE(1, (PHY_ISOLATE | PHY_AD2 | PHY_CONFIG2), (0), (PORT_SRE_SLOW | PORT_DSE_MID))
        #define FNFORCE_PHY_CONFIG()       _SETBITS(3, RESET_PHY); fnDelayLoop(10000) // take PHY out of reset and wait 10ms so that it will be ready for following operations
    ...
    #else
        #define ETHERNET_RMII                                            // use reduced MII interface
            ...
            #define FNFORCE_PHY_CONFIG()   _SETBITS(1, RESET_PHY); fnDelayLoop(10000); _CONFIG_PORT_INPUT(1, PHY_INTERRUPT_OUTPUT, PORT_PS_UP_ENABLE) // take PHY out of reset and wait 10ms so that it will be ready for following operations
        #else
            #define RESET_PHY              USER_LED                      // control the PHY's reset so that its configuration can be controlled
          //#define PHY_ISOLATE            PORT2_BIT27
            #define PHY_INTERRUPT_OUTPUT   PIN_GPIO_AD_B0_10_GPIO1_IO10
            #define ASSERT_PHY_RST()       
            #define CONFIG_PHY_STRAPS()    _CONFIG_DRIVE_PORT_OUTPUT_VALUE(1, (PHY_INTERRUPT_OUTPUT), (PHY_INTERRUPT_OUTPUT), (PORT_SRE_SLOW | PORT_DSE_MID)); /*_CONFIG_DRIVE_PORT_OUTPUT_VALUE(2, (PHY_ISOLATE), (0), (PORT_SRE_SLOW | PORT_DSE_MID))*/
            #define FNFORCE_PHY_CONFIG()   _CONFIG_DRIVE_PORT_OUTPUT_VALUE(1, (RESET_PHY), (0), (PORT_SRE_SLOW | PORT_DSE_MID)); fnDelayLoop(10000); _SETBITS(1, RESET_PHY); _CONFIG_PORT_INPUT(1, PHY_INTERRUPT_OUTPUT, PORT_PS_UP_ENABLE) // take PHY out of reset and wait 10ms so that it will be ready for following operations
        #endif
        #define FORCE_PHY_CONFIG
  ...
  //#define MSCR_HOLDTIME          7
    ...
    #define CONFIG_PHY_STRAPS()        _CONFIG_DRIVE_PORT_OUTPUT_VALUE(2, (PHY_STRAP_A0 | PHY_STRAP_A1), (PHY_STRAP_A1), (PORT_SRE_SLOW | PORT_DSE_MID))
...
#define SUPPORT_TIMER                                                    // support hardware timer interrupt configuration (GPT)
    #define GPT_PROJECT_MODE       (GPT_DBGEN | GPT_WAITEN | GPT_DOZEEN | GPT_STOPEN) // GPT remains enabled in debug, wait, doze and stop modes
...
#if (defined MIMXRT1050 || defined iMX_RT1052_EMB_ART || defined iMX_RT1062_EMB_ART || defined MIMXRT1064) && defined SUPPORT_GLCD
    #define GLCD_BACKLIGHT_CONTROL                                       // enable backlight intensity control
    #define SDRAM_CONFIGURED_BY_DCD                                      // used when the boot loader has already configured SDRAM
    #define SEMC_CLK_FROM_PLL2_PFD2                                      // SEMC clock taken from PLL-PFD2
    #undef PLL2_PFD2_FRACTION
    #define PLL2_PFD2_FRACTION            29                             // PLL2-PFD2 frequency of 327.7241379MHz

    #define LCD_RST                       PIN_GPIO_AD_B0_02_GPIO1_IO02
    #define LCD_BACKLIGHT                 PIN_GPIO_B1_15_GPIO2_IO31
    #define _GLCD_BACKLIGHT_TIMER         (_FLEX_PWM_4 | _FLEX_PWM_A_3)
    #define _GLCD_TIMER_MODE_OF_OPERATION (PWM_SYS_CLK | PWM_PRESCALER_16 | PWM_EDGE_ALIGNED)
    #define _GLCD_BACKLIGHT_PWM_FREQUENCY PWM_FREQUENCY(20000, 16)
    #define BACK_LIGHT_MAX_INTENSITY()    _CONFIG_DRIVE_PORT_OUTPUT_VALUE(2, (LCD_BACKLIGHT), (LCD_BACKLIGHT), (PORT_SRE_SLOW | PORT_DSE_LOW))
    #define BACK_LIGHT_MIN_INTENSITY()    _CONFIG_DRIVE_PORT_OUTPUT_VALUE(2, (LCD_BACKLIGHT), (0), (PORT_SRE_SLOW | PORT_DSE_LOW))
    #if defined MIMXRT1064
        #define eLCD_IN_OCR2                                             // locate the frame buffer in OCR2 rather than in SDRAM
        #define eLCD_IN_16_BIT_MODE                                      // use 16 bit more rather than 24 it mode
    #endif
    // Touch screen interrupt
    //
    #define TC_INT_PRIORTY                PRIORITY_PORT_1_LOW
    #define TC_INT_PORT                   PORT1
    #define TC_INT_PORT_BIT               PIN_GPIO_AD_B0_11_GPIO1_IO11
    #define TOUCH_INTERRUPT_STATE()       _READ_PORT_MASK(1, TC_INT_PORT_BIT)
    #define TOUCHRESET_H()                                               // no reset line present
#endif
...
#define TICK_INTERRUPT()                                                 // user callback from system TICK (dummy if left empty)
  ...
  //#define SPI_FLASH_S26KS512                                           // 64MBytes
    ...
    #define SPI_FLASH_SECTOR_LENGTH      (256 * 1024)                    // block size of SPI FLASH (there are 8 optional 4k parameter sectors too)
...
#define SW_UPLOAD_FILE()       (*ucIp_Data == 'H') && (*(ucIp_Data+1) == 'S') && (*(ucIp_Data+2) == '.')  && (fnSPI_Flash_available() != NO_SPI_FLASH_AVAILABLE)

// FLASH based File System setup
//
#if defined FLASH_FILE_SYSTEM && defined SPI_FILE_SYSTEM                 // setup for external SPI FLASH, with the parameters at the end of internal FLASH
    #if defined SPI_FLASH_S26KL
        #define uFILE_START          (FLEXSPI_FLASH_BASE + (1024 * 1024))// after the boot, serial loader and application space
        #define FILE_GRANULARITY     (256 * 1024)                        // block size
        #define FILE_SYSTEM_SIZE     (FILE_GRANULARITY * 70)             // 17.92MBytes
    #elif defined TEENSY_4_0 || defined TEENSY_4_1
        #define uFILE_START          (FLEXSPI_FLASH_BASE + 0xa0000)      // after the boot, serial loader and application space
        #define FILE_GRANULARITY     (24 * 1024)                         // suitable for 2MByte SPI flash with 4k flash granularity
        #define FILE_SYSTEM_SIZE     (0x100000)                          // 1MBytes
        #define FILE_GRANULARITY_NOT_MODULO                              // enabled because file granularity is not modulo in size
    #elif defined iMX_RT1052_EMB_ART || defined iMX_RT1062_EMB_ART || (defined MIMXRT1064 && !defined USE_EXTERNAL_QSPI_FLASH)
        #define uFILE_START          (FLEXSPI_FLASH_BASE + 0xa0000)      // after the boot, serial loader and application space
        #define FILE_GRANULARITY     (48 * 1024)                         // suitable for 4MByte SPI flash with 4k flash granularity
        #define FILE_SYSTEM_SIZE     (0x30c000)                          // 3.120MBytes
        #define FILE_GRANULARITY_NOT_MODULO                              // enabled because file granularity is not modulo in size
    #else
        #define uFILE_START          (FLEXSPI_FLASH_BASE + 0xa0000)      // after the boot, serial loader and application space
        #define FILE_GRANULARITY     (112 * 1024)                        // suitable for 8MByte SPI flash (and larger) with 4k flash granularity
        #define FILE_SYSTEM_SIZE     (0x734000)                          // 7.376MBytes
        #define FILE_GRANULARITY_NOT_MODULO                              // enabled because file granularity is not modulo in size
    #endif
#endif
    ...
    #if defined SPI_FLASH_S26KL
        #define PARAMETER_BLOCK_GRANULARITY (256 * 1024)                 // block size
    #else
        #define PARAMETER_BLOCK_GRANULARITY (32 * 1024)                  // sub-sector size
    #endif
    #define PARAMETER_BLOCK_SIZE            PARAMETER_BLOCK_GRANULARITY
        ...
        #define PARAMETER_BLOCK_START       (FLEXSPI_FLASH_BASE + SPI_FLASH_SIZE - PAR_BLOCK_SIZE - (64 * 1024)) // 128k before the end of QSPI flash in order to not conflict with the teensy 4 recovery code
    #else
        #define PARAMETER_BLOCK_START       (FLEXSPI_FLASH_BASE + SPI_FLASH_SIZE - PAR_BLOCK_SIZE) // 64k before the end of QSPI flash
    #endif
...
//
    ...
    #define SERIAL_PORT_3    10                                          // if we open UART channel 3 we simulate using comx on the PC
    #define SERIAL_PORT_4    6                                           // if we open UART channel 4 we simulate using comx on the PC
    #define SERIAL_PORT_5    6                                           // if we open UART channel 5 we simulate using comx on the PC
    #define SERIAL_PORT_6    6                                           // if we open UART channel 6 we simulate using comx on the PC
    #define SERIAL_PORT_7    6                                           // if we open UART channel 7 we simulate using comx on the PC

    #if defined TEENSY_4_0 || defined TEENSY_4_1
        #define DEMO_UART        iMX_LPUART_3                            // LPUART3
        #define LPUART3_ON_EMC
    #elif defined iMX_RT1062_EMB_ART && defined DEV9
        #define DEMO_UART        iMX_LPUART_2                            // LPUART2
    #else
        #define DEMO_UART        iMX_LPUART_1                            // LPUART1
    #endif
  ...
  #define SERIAL_SUPPORT_DMA                                             // enable UART DMA support (TX)
    ...
    #define TX_BUFFER_SIZE   (QUEUE_TRANSFER)(4 * 1024)                  // the size of RS232 output buffer
    #define RX_BUFFER_SIZE   (128)                                       // the size of RS232 iput buffer
...
//
      ...
      //#define SUPPORT_DAC                                              // enable general DAC support
...
// Define behavior of low power PIT (when available) in debug and doze mode
    ...
    #define SUPPORT_PITS                                                 // support PITs so that they can be used for modbus RTU timing
#endif

#if defined SUPPORT_TIMER
    #define SUPPORT_PWM_MODULE                                           // enable PWM support from FlexPWM
#endif
...
//
    ...
    #if defined MIMXRT1010 || defined MIMXRT1015 || defined MIMXRT1050 || defined MIMXRT1060 || defined MIMXRT1064
...
//
    ...
    #define NUMBER_OF_TX_BUFFERS_IN_ETHERNET_DEVICE         2            // allocate 2 buffers for transmission
        ...
        #define NUMBER_OF_RX_BUFFERS_IN_ETHERNET_DEVICE     40           // allocate 40 (full) buffers for reception
    #else
...
#endif
#if defined ETH_INTERFACE || defined WIFI_INTERFACE || defined USB_CDC_RNDIS || defined USE_PPP || defined USB_RNDIS_HOST
    ...
    //
        ...
        #define FTP_WELCOME_RESPONSE  "220 Welcome i.MX RT FTP.\r\n"
...
//
    ...
    #define DEMO_LED_1             (USER_LED)                            // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_2             (PIN_GPIO_AD_01_GPIO1_IO15)           // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_3             (PIN_GPIO_AD_02_GPIO1_IO16)           // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_4             (PIN_GPIO_AD_03_GPIO1_IO17)
    #define BLINK_LED              DEMO_LED_1

    #define USER_BUTTON            PIN_GPIO_SD_05_GPIO2_IO05
    ...
    #else
        #define ACTIVATE_WATCHDOG()     UNLOCK_WDOG3(); WDOG3_TOVAL = (2 * 32000); WDOG3_WIN = 0; WDOG3_CS = (WDOG_CS_INT | WDOG_CS_CLK_LPO | WDOG_CS_FLG | WDOG_CS_CMD32EN | WDOG_CS_EN); // enable watchdog with 2s timeout (32kHz reference)
    #endif

    #define SHIFT_DEMO_LED_1        11                                   // since the port bits are spread out shift each to the lowest 4 bits
    #define SHIFT_DEMO_LED_2        14
    ...
    #define INIT_WATCHDOG_DISABLE() _CONFIG_PORT_INPUT(2, USER_BUTTON, PORT_PS_UP_ENABLE) // use fast access version (beware that this can only operate on half of the 32 bits at a time)
    ...
    #define KEYPAD "KeyPads/MIMXRT1010-EVK.bmp"
    ...
    #define KEYPAD_LED_DEFINITIONS  \
        {RGB(0,0,0), RGB(0, 255, 0),  0,    {235, 227, 243, 236}, _PORT1, USER_LED}
    ...
    #define MOUSE_LEFT_CLICK()     (_READ_PORT_MASK(2, USER_BUTTON) == 0)
    #define MOUSE_UP()             (0)
    #define MOUSE_DOWN()           (0)
    #define MOUSE_LEFT()           (0)
    #define MOUSE_RIGHT()          (0)
#elif defined MIMXRT1015
    #define USER_LED               (PORT3_BIT20)                         // USER_LED ('0' drives LED on)

    #define DEMO_LED_1             (USER_LED)                            // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_2             (PORT3_BIT21)                         // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_3             (PORT3_BIT22)                         // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_4             (PORT3_BIT23)
    #define BLINK_LED              DEMO_LED_1

    #define USER_BUTTON            PORT2_BIT9
    #define USER_BUTTON_PORT       _PORT2

    #if defined USE_MAINTENANCE && !defined REMOVE_PORT_INITIALISATIONS
        #define INIT_WATCHDOG_LED()                                      // let the port set up do this (the user can disable blinking)
    #else
        #define INIT_WATCHDOG_LED() _CONFIG_DRIVE_PORT_OUTPUT_VALUE(3, (BLINK_LED | DEMO_LED_2), (BLINK_LED), (PORT_SRE_SLOW | PORT_DSE_HIGH))
    #endif
    ...
    #define SHIFT_DEMO_LED_1        20                                   // since the port bits may be spread out shift each to the lowest 4 bits
    #define SHIFT_DEMO_LED_2        20
    #define SHIFT_DEMO_LED_3        20
    #define SHIFT_DEMO_LED_4        20
    ...
    #define CONFIG_TEST_OUTPUT()                                         // we use DEMO_LED_2 which is configured by the user code (and can be disabled in parameters if required)
    #define TOGGLE_TEST_OUTPUT()    _TOGGLE_PORT(3, DEMO_LED_2)
    #define SET_TEST_OUTPUT()       _SETBITS(3, DEMO_LED_2)
    #define CLEAR_TEST_OUTPUT()     _CLEARBITS(3, DEMO_LED_2)

    #define KEYPAD "KeyPads/MIMXRT1015-EVK.bmp"
    ...
    #define KEYPAD_LED_DEFINITIONS  \
        ...
        {RGB(0, 255, 0),  RGB(0,0,0), 1,    {189, 198, 196, 209}, _PORT3, DEMO_LED_2}, \
        {RGB(0, 255, 0),  RGB(0,0,0), 1,    {197, 198, 203, 209}, _PORT3, DEMO_LED_3}
    ...
    #define DEMO_LED_1             (USER_LED)                            // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_2             (PORT1_BIT31)                         // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_3             (PORT1_BIT18)                         // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_4             (PORT1_BIT19)
    #define BLINK_LED              DEMO_LED_1
    ...
    #if defined MIMXRT1024
        #define KEYPAD    "KeyPads/MIMXRT1024-EVK.bmp"
    #else
    ...
    #define CONFIGURE_MOUSE_INPUTS()
    #define MOUSE_LEFT_CLICK()     (_READ_PORT_MASK(5, USER_BUTTON) == 0)
    #define MOUSE_UP()             (0)
    #define MOUSE_DOWN()           (0)
    #define MOUSE_LEFT()           (0)
    #define MOUSE_RIGHT()          (0)
#elif defined MIMXRT1050 || defined MIMXRT1060 || defined iMX_RT1052_EMB_ART || defined iMX_RT1062_EMB_ART || defined MIMXRT1170
    #define USER_LED               (PIN_GPIO_AD_B0_09_GPIO1_IO09)        // USER_LED ('0' drives LED on)

    #define DEMO_LED_1             (USER_LED)                            // if the port is changed (eg. GPIO1 to GPIO2) the port macros will require appropriate adjustment too
    #if defined MIMXRT1060
        #define DEMO_LED_2         (PIN_GPIO_AD_B0_03_GPIO1_IO03)        // J24-1 - if the port is changed (eg. GPIO1 to GPIO2) the port macros will require appropriate adjustment too
    #else
        #define DEMO_LED_2         (PIN_GPIO_AD_B1_15_GPIO1_IO31)        // if the port is changed (eg. GPIO1 to GPIO2) the port macros will require appropriate adjustment too
    #endif
    #define DEMO_LED_3             (PIN_GPIO_AD_B1_02_GPIO1_IO18)        // if the port is changed (eg. GPIO1 to GPIO2) the port macros will require appropriate adjustment too
    #define DEMO_LED_4             (PIN_GPIO_AD_B1_03_GPIO1_IO19)        // if the port is changed (eg. GPIO1 to GPIO2) the port macros will require appropriate adjustment too
    #define BLINK_LED              DEMO_LED_1
    ...
    #if defined SPI_FLASH_S26KL || defined SPI_FLASH_ATXP
        #define WATCHDOG_TIMEOUT_SECS  4                                 // longer timeout due to slow flash block erase
    #else
        #define WATCHDOG_TIMEOUT_SECS  2
    #endif

    #if defined iMX_BOOTLOADER
        #define ACTIVATE_WATCHDOG()     UNLOCK_WDOG3(); WDOG3_TOVAL = ((WATCHDOG_TIMEOUT_SECS) * 32000); WDOG3_WIN = 0; WDOG3_CS = (WDOG_CS_CLK_LPO | WDOG_CS_FLG | WDOG_CS_CMD32EN | WDOG_CS_EN); // enable watchdog with WATCHDOG_TIMEOUT_SECS timeout (32kHz reference)
    #else
        #define ACTIVATE_WATCHDOG()     UNLOCK_WDOG3(); WDOG3_TOVAL = ((WATCHDOG_TIMEOUT_SECS) * 32000); WDOG3_WIN = 0; WDOG3_CS = (WDOG_CS_INT | WDOG_CS_CLK_LPO | WDOG_CS_FLG | WDOG_CS_CMD32EN | WDOG_CS_EN); // enable watchdog with WATCHDOG_TIMEOUT_SECS timeout (32kHz reference)
    #endif
    ...
    #if defined MIMXRT1170
        #define KEYPAD "KeyPads/MIMXRT1170-EVK.bmp"
    #elif defined MIMXRT1060
        #define KEYPAD "KeyPads/MIMXRT1060-EVK.bmp"
    #elif defined iMX_RT1052_EMB_ART || defined iMX_RT1062_EMB_ART
        #define KEYPAD "KeyPads/EmbeddedArt_OEM.bmp"
    #else
        #define KEYPAD "KeyPads/MIMXRT1050-EVK.bmp"
    #endif
    ...
    #define BUTTON_KEY_DEFINITIONS  {USER_BUTTON_PORT, USER_BUTTON,   {167, 97, 185, 113 }},
        ...
        // '0'          '1'           input state   center (x,   y)   0 = circle, radius, controlling port, controlling pin 
    #define KEYPAD_LED_DEFINITIONS  \
        {RGB(0, 255,  0  ), RGB(0,0,0),  1, {382, 212,  389, 223 }, _PORT1, USER_LED}

    extern unsigned char FlexSPI_CS;
    #define SPI_CS0_PORT FlexSPI_CS
    #define CS0_LINE     1

    #define CANopen_CHANNEL        iMX_FLEXCAN_2
    ...
    #define MOUSE_RIGHT()          (0)
        ...
        #define WIFI_RESET_OUTPUT      PIN_GPIO_AD_B1_15_GPIO1_IO31
        #define WIFI_ENABLE_OUTPUT     PIN_GPIO_AD_B1_14_GPIO1_IO30
        #define WIFI_IRQ_INPUT         PIN_GPIO_AD_B1_13_GPIO1_IO29
        #define WIFI_IRQ_INPUT_PORT    PORT1
        #define WIFI_IRQ_PRIORITY      PRIORITY_PORT_1_HIGH
        #define WIFI_WAKEUP_OUTPUT     PIN_GPIO_AD_B1_12_GPIO1_IO28

        #define WIFI_CS_OUT            PIN_GPIO_SD_B0_01_GPIO3_IO13

        // Configure outputs and SPI, enable the wifi module but initially hold it in reset
        // - configure for SPI mode 00
        #define CONFIGURE_INTERFACE_WIFI() _CONFIG_DRIVE_PORT_OUTPUT_VALUE(1, (WIFI_RESET_OUTPUT | WIFI_ENABLE_OUTPUT | WIFI_WAKEUP_OUTPUT), (WIFI_ENABLE_OUTPUT | WIFI_WAKEUP_OUTPUT), (PORT_SRE_SLOW | PORT_DSE_MID)); \
                                        _CONFIG_DRIVE_PORT_OUTPUT_VALUE(3, (WIFI_CS_OUT), (WIFI_CS_OUT), (PORT_SRE_FAST | PORT_DSE_MID)); \
                                        ...
                                        LPSPI1_TCR = (LPSPI_TCR_PRESCALE_1 | LPSPI_TCR_MSBF | (8 - 1)); \
                                        LPSPI1_CR = (LPSPI_CR_MEN | LPSPI_CR_DBGEN)

        #define READ_WIFI_DATA()        (unsigned char)LPSPI1_RDR
        #define WRITE_WIFI_DATA(byte)   LPSPI1_TDR = (byte)              // write a single byte
        #define WAIT_WIFI_TX_END()      _WAIT_REGISTER_TRUE(LPSPI1_RSR, LPSPI_RSR_RXEMPTY) // wait until the complete byte has been sent and the received byte is ready

        #define ASSERT_RESET_WIFI()    _CLEARBITS(1, WIFI_RESET_OUTPUT)
        #define NEGATE_RESET_WIFI()    _SETBITS(1, WIFI_RESET_OUTPUT)

        #define ASSERT_CS_WIFI()       _CLEARBITS(3, WIFI_CS_OUT)
        #define NEGATE_CS_WIFI()       _SETBITS(3, WIFI_CS_OUT)

        extern void fnATWINC15_wdog(void);
        #define USER_WATCHDOG_FUNCTION()   fnATWINC15_wdog()             // flash WIFI chip LED when operating correctly
    #endif
#elif defined ARCH_MIX
    #define DEMO_LED_1             (PORT1_BIT10)                         // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_2             (PORT1_BIT9)                          // red led - if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_3             (PORT1_BIT11)                         // blue led - if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_4             (0)
    #define BLINK_LED              DEMO_LED_1
    ...
    #define KEYPAD "KeyPads/ArchMix.bmp"
    ...
    #define DEMO_LED_1             (USER_LED)                            // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_2             (PIN_GPIO_AD_B1_15_GPIO1_IO31)        // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_3             (PIN_GPIO_AD_B0_10_GPIO1_IO10)                         // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_4             (PIN_GPIO_AD_B0_11_GPIO1_IO11)
    #define BLINK_LED              DEMO_LED_1
    ...
    #define USER_BUTTON_PORT       _PORT5
        ...
        #define INIT_WATCHDOG_LED()                                      // let the port set up do this (the user can disable blinking)
    #else
        #define INIT_WATCHDOG_LED() _CONFIG_DRIVE_PORT_OUTPUT_VALUE(6, (BLINK_LED | DEMO_LED_2), (BLINK_LED), (PORT_SRE_SLOW | PORT_DSE_HIGH)) // note that port 6 is specified so that fast port is used
    #endif
    ...
    #define INIT_WATCHDOG_DISABLE() _CONFIG_PORT_INPUT(1, PORT1_BIT17, PORT_PS_UP_ENABLE) // configure an input which can be used to disable the watchog
    #define WATCHDOG_DISABLE()      (_READ_PORT_MASK(1, PORT1_BIT17) == 0) // pull this input down to disable watchdog (J24-9)

    #define TOGGLE_WATCHDOG_LED()   _TOGGLE_PORT(6, BLINK_LED)

    #define CONFIG_TEST_OUTPUT()                                         // we use DEMO_LED_2 which is configured by the user code (and can be disabled in parameters if required)
    #define TOGGLE_TEST_OUTPUT()    _TOGGLE_PORT(6, DEMO_LED_2)
    #define SET_TEST_OUTPUT()       _SETBITS(6, DEMO_LED_2)
    #define CLEAR_TEST_OUTPUT()     _CLEARBITS(6, DEMO_LED_2)

    #define KEYPAD "KeyPads/MIMXRT1064-EVK.bmp"

    #define BUTTON_KEY_DEFINITIONS  {USER_BUTTON_PORT, USER_BUTTON,   {155, 98, 169, 113 }},
        // '0'          '1'           input state   center (x,   y)   0 = circle, radius, controlling port, controlling pin 
    #define KEYPAD_LED_DEFINITIONS  \
        {RGB(0, 255,  0  ), RGB(0,0,0),  1, {344, 200,  350, 209 }, _PORT1, USER_LED}
    ...
    #define MOUSE_RIGHT()          (0)
#elif defined TEENSY_4_0 || defined TEENSY_4_1
    #define USER_LED               (PORT2_BIT3)                          // USER_LED ('1' drives LED on)

    #define DEMO_LED_1             (USER_LED)                            // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_2             (PORT2_BIT11)                         // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_3             (PORT2_BIT16)                         // if the port is changed (eg. 1 to 2) the port macros will require appropriate adjustment too
    #define DEMO_LED_4             (PORT2_BIT17)
    #define BLINK_LED              DEMO_LED_1

    #define USER_BUTTON            PORT1_BIT25
    #define USER_BUTTON_PORT       _PORT1

    #if defined USE_MAINTENANCE && !defined REMOVE_PORT_INITIALISATIONS
        #define INIT_WATCHDOG_LED()                                      // let the port set up do this (the user can disable blinking)
    #else
        #define INIT_WATCHDOG_LED() _CONFIG_DRIVE_PORT_OUTPUT_VALUE(2, (BLINK_LED | DEMO_LED_2), (BLINK_LED), (PORT_SRE_SLOW | PORT_DSE_HIGH))
    #endif
    ...
    #define SHIFT_DEMO_LED_1        3                                    // since the port bits are spread out shift each to the lowest 4 bits
    #define SHIFT_DEMO_LED_2        10
    #define SHIFT_DEMO_LED_3        14
    #define SHIFT_DEMO_LED_4        14
    ...
    #define INIT_WATCHDOG_DISABLE() _CONFIG_PORT_INPUT(1, USER_BUTTON, PORT_PS_UP_ENABLE) // use fast access version (beware that this can only operate on half of the 32 bits at a time)
    ...
    #define CONFIG_TEST_OUTPUT()                                         // we use DEMO_LED_2 which is configured by the user code (and can be disabled in parameters if required)
    #define TOGGLE_TEST_OUTPUT()    _TOGGLE_PORT(2, DEMO_LED_2)
    #define SET_TEST_OUTPUT()       _SETBITS(2, DEMO_LED_2)
    #define CLEAR_TEST_OUTPUT()     _CLEARBITS(2, DEMO_LED_2)

    #if defined TEENSY_4_1
        #define KEYPAD "KeyPads/Teensy_4-1.bmp"

            // '0'          '1'           input state   center (x,   y)   0 = circle, radius, controlling port, controlling pin 
        #define KEYPAD_LED_DEFINITIONS  \
            {RGB(0, 0,  0  ), RGB(255,128,0),  0, {468, 97,  478, 119 }, _PORT2, USER_LED},
    #else
        #define KEYPAD "KeyPads/Teensy_4-0.bmp"

        #define BUTTON_KEY_DEFINITIONS  {USER_BUTTON_PORT, USER_BUTTON,   {122,   9, 141,  24 }}, \
                                        {_PORT1,           PORT1_BIT3,    { 58, 192,  72, 209 }}, \
            ...
            // '0'          '1'           input state   center (x,   y)   0 = circle, radius, controlling port, controlling pin 
        #define KEYPAD_LED_DEFINITIONS  \
            {RGB(0, 0,  0  ), RGB(255,128,0),  0, {413, 51,  426, 71 }, _PORT2, USER_LED}, \
            {RGB(0, 0,  0  ), RGB(255,0,  0),  0, {443, 17,  0,    3 }, _PORT2, USER_LED},
    #endif
    ...
    #define CONFIGURE_MOUSE_INPUTS()
    #define MOUSE_LEFT_CLICK()     (_READ_PORT_MASK(1, USER_BUTTON) == 0)
    #define MOUSE_UP()             (0)
    #define MOUSE_DOWN()           (0)
    #define MOUSE_LEFT()           (0)
    #define MOUSE_RIGHT()          (0)
...
// User port mapping
//
#define USER_PORT_1_BIT            PORT1_BIT25                            // use free pins on Eval board
#define USER_PORT_2_BIT            PORT1_BIT24
#define USER_PORT_3_BIT            PORT1_BIT27
#define USER_PORT_4_BIT            PORT2_BIT0
#define USER_PORT_5_BIT            PORT1_BIT28
#define USER_PORT_6_BIT            PORT2_BIT3
#define USER_PORT_7_BIT            PORT1_BIT21
#define USER_PORT_8_BIT            PORT1_BIT22
#define USER_PORT_9_BIT            PORT1_BIT23
#define USER_PORT_10_BIT           PORT1_BIT29
#define USER_PORT_11_BIT           PORT1_BIT18
#define USER_PORT_12_BIT           PORT1_BIT19
#define USER_PORT_13_BIT           PORT1_BIT20
#define USER_PORT_14_BIT           PORT1_BIT26
#define USER_PORT_15_BIT           PORT2_BIT1
#define USER_PORT_16_BIT           PORT2_BIT2

#define _CONFIG_OUTPUT_PORT_1()    _CONFIG_PORT_OUTPUT(1, USER_PORT_1_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_2()    _CONFIG_PORT_OUTPUT(1, USER_PORT_2_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_3()    _CONFIG_PORT_OUTPUT(1, USER_PORT_3_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_4()    _CONFIG_PORT_OUTPUT(2, USER_PORT_4_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_5()    _CONFIG_PORT_OUTPUT(1, USER_PORT_5_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_6()    _CONFIG_PORT_OUTPUT(2, USER_PORT_6_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_7()    _CONFIG_PORT_OUTPUT(1, USER_PORT_7_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_8()    _CONFIG_PORT_OUTPUT(1, USER_PORT_8_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_9()    _CONFIG_PORT_OUTPUT(1, USER_PORT_9_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_10()   _CONFIG_PORT_OUTPUT(1, USER_PORT_10_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_11()   _CONFIG_PORT_OUTPUT(1, USER_PORT_11_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_12()   _CONFIG_PORT_OUTPUT(1, USER_PORT_12_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_13()   _CONFIG_PORT_OUTPUT(1, USER_PORT_13_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_14()   _CONFIG_PORT_OUTPUT(1, USER_PORT_14_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_15()   _CONFIG_PORT_OUTPUT(2, USER_PORT_15_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
#define _CONFIG_OUTPUT_PORT_16()   _CONFIG_PORT_OUTPUT(2, USER_PORT_16_BIT, (PORT_SRE_SLOW | PORT_DSE_LOW))
...
#define USER_DDR_3             GPIO1_GDIR

#define USER_SET_PORT_4        GPIO2_DR_SET
#define USER_CLEAR_PORT_4      GPIO2_DR_CLEAR
#define USER_PORT_4            GPIO2_DR
#define USER_TOGGLE_PORT_4     GPIO2_DR_TOGGLE
#define USER_DDR_4             GPIO2_GDIR

#define USER_SET_PORT_5        GPIO1_DR_SET
#define USER_CLEAR_PORT_5      GPIO1_DR_CLEAR
#define USER_PORT_5            GPIO1_DR
#define USER_TOGGLE_PORT_5     GPIO1_DR_TOGGLE
#define USER_DDR_5             GPIO1_GDIR

#define USER_SET_PORT_6        GPIO2_DR_SET
#define USER_CLEAR_PORT_6      GPIO2_DR_CLEAR
#define USER_PORT_6            GPIO2_DR
#define USER_TOGGLE_PORT_6     GPIO2_DR_TOGGLE
#define USER_DDR_6             GPIO2_GDIR

#define USER_SET_PORT_7        GPIO1_DR_SET
#define USER_CLEAR_PORT_7      GPIO1_DR_CLEAR
#define USER_PORT_7            GPIO1_DR
#define USER_TOGGLE_PORT_7     GPIO1_DR_TOGGLE
#define USER_DDR_7             GPIO1_GDIR

#define USER_SET_PORT_8        GPIO1_DR_SET
#define USER_CLEAR_PORT_8      GPIO1_DR_CLEAR
#define USER_PORT_8            GPIO1_DR
#define USER_TOGGLE_PORT_8     GPIO1_DR_TOGGLE
#define USER_DDR_8             GPIO1_GDIR

#define USER_SET_PORT_9        GPIO1_DR_SET
#define USER_CLEAR_PORT_9      GPIO1_DR_CLEAR
#define USER_PORT_9            GPIO1_DR
#define USER_TOGGLE_PORT_9     GPIO1_DR_TOGGLE
#define USER_DDR_9             GPIO1_GDIR

#define USER_SET_PORT_10       GPIO1_DR_SET
#define USER_CLEAR_PORT_10     GPIO1_DR_CLEAR
#define USER_PORT_10           GPIO1_DR
#define USER_TOGGLE_PORT_10    GPIO1_DR_TOGGLE
#define USER_DDR_10            GPIO1_GDIR

#define USER_SET_PORT_11       GPIO1_DR_SET
#define USER_CLEAR_PORT_11     GPIO1_DR_CLEAR
#define USER_PORT_11           GPIO1_DR
#define USER_TOGGLE_PORT_11    GPIO1_DR_TOGGLE
#define USER_DDR_11            GPIO1_GDIR

#define USER_SET_PORT_12       GPIO1_DR_SET
#define USER_CLEAR_PORT_12     GPIO1_DR_CLEAR
#define USER_PORT_12           GPIO1_DR
#define USER_TOGGLE_PORT_12    GPIO1_DR_TOGGLE
#define USER_DDR_12            GPIO1_GDIR

#define USER_SET_PORT_13       GPIO1_DR_SET
#define USER_CLEAR_PORT_13     GPIO1_DR_CLEAR
#define USER_PORT_13           GPIO1_DR
#define USER_TOGGLE_PORT_13    GPIO1_DR_TOGGLE
#define USER_DDR_13            GPIO1_GDIR

#define USER_SET_PORT_14       GPIO1_DR_SET
#define USER_CLEAR_PORT_14     GPIO1_DR_CLEAR
#define USER_PORT_14           GPIO1_DR
#define USER_TOGGLE_PORT_14    GPIO1_DR_TOGGLE
#define USER_DDR_14            GPIO1_GDIR

#define USER_SET_PORT_15       GPIO2_DR_SET
#define USER_CLEAR_PORT_15     GPIO2_DR_CLEAR
#define USER_PORT_15           GPIO2_DR
#define USER_TOGGLE_PORT_15    GPIO2_DR_TOGGLE
#define USER_DDR_15            GPIO2_GDIR

#define USER_SET_PORT_16       GPIO2_DR_SET
#define USER_CLEAR_PORT_16     GPIO2_DR_CLEAR
#define USER_PORT_16           GPIO2_DR
#define USER_TOGGLE_PORT_16    GPIO2_DR_TOGGLE
#define USER_DDR_16            GPIO2_GDIR
...
#define USB_DOWNLOAD_FILE      "z"                                       // software upload via USB-CDC is saved to this internal file location
  ...
  //#define SDCARD_USES_SPI                                              // use LPSPI interface rather than SDHC
  ...
  //#define _NO_SD_CARD_INSERTED                                         // simulate no SD card inserted initially
      ...
      //#define SDCARD_DETECT_INPUT_POLL                                 // use card detect switch for detection polling (use together with T_CHECK_CARD_REMOVAL)
    ...
    #if NUMBER_OF_SDHC > 0 && !defined SDCARD_USES_SPI                   // use SDHC controller
        ...
        #if NUMBER_OF_SDHC > 1
          //#define USE_uSDHC2                                           // use SDHC2 instead of SDHC1
          //#define uSDHC2_ON_SD                                         // uSDHC2 connected on GPIO_SD_B1_XX connection set rather tha GPIO_AD_B1_XX set
            #define uSDHC2_8_BIT_BUS                                     // use full 8 bit data bus rather than 4 bit bus
        #endif
        ...
        #define SET_SD_CS_LOW()                                          // dummy since not required by SD controller
            ...
            #define SD_PWREN               PIN_GPIO_AD_B0_05_GPIO1_IO05
            #define POWER_TO_SD_CARD()     _CONFIG_DRIVE_PORT_OUTPUT_VALUE(1, SD_PWREN, SD_PWREN, (PORT_SRE_SLOW | PORT_DSE_LOW))
            #define POWER_DOWN_SD_CARD()   //  _CLEARBITS(1, SD_PWREN)   // remove power from SD card interface - not removed on this board since the power is needed for card detection
        #elif defined MIMXRT1060 || defined MIMXRT1064
            #define SD_PWREN               PIN_GPIO_AD_B1_03_GPIO1_IO19 
            #define POWER_TO_SD_CARD()     _CONFIG_DRIVE_PORT_OUTPUT_VALUE(1, SD_PWREN, SD_PWREN, (PORT_SRE_SLOW | PORT_DSE_LOW))
            #define POWER_DOWN_SD_CARD()   _CLEARBITS(1, SD_PWREN)       // remove power from SD card interface
        #elif defined TEENSY_4_0 || defined TEENSY_4_1
    ...
    //#define HEAP2_ADDRESS     SDRAM_ADDR                               // DRAM start address in memory map
    //#define HEAP2_SIZE        SDRAM_SIZE
...
//
    ...
    #define DEL_USB_SYMBOL()          fnDisplayInterface(0, 0)           // control display of USB enumeration - clear
    #define SET_USB_SYMBOL()          fnDisplayInterface(0, 1)           // control display of USB enumeration - set
    #define DEL_ETH_SYMBOL()          fnDisplayInterface(1, 0)           // control display of ethernet connection - clear
    #define SET_ETH_SYMBOL()          fnDisplayInterface(1, 1)           // control display of ethernet connection - set
#else
    ...
    #define DEL_ETH_SYMBOL()                                             // control display of ethernet connection - clear
    #define SET_ETH_SYMBOL()                                             // control display of ethernet connection - set
#endif

#if defined _GLCD_SAMSUNG
    #define DATA_LINES                 (PORT1_BIT7 | PORT1_BIT6 | PORT1_BIT5 | PORT1_BIT4 | PORT1_BIT3 | PORT1_BIT2 | PORT1_BIT1 | PORT1_BIT0)

    #define GLCD_RST                   PORT1_BIT8                        // reset
    #define GLCD_RS                    PORT1_BIT9                        // GLCD register select
    #define GLCD_RW                    PORT1_BIT10                       // GLCD read/write
    #define GLCD_ENA                   PORT1_BIT11                       // GLCD enable

    #define GLCD_CS0                   PORT1_BIT12                       // LCD controller 0 chip select - 2 controller chips for 128 x 64
    #define GLCD_CS1                   PORT1_BIT13                       // LCD controller 1 chip select

    #define SET_PULL_DOWNS()       
    #define REMOVE_PULL_DOWNS()    

    #define CONFIGURE_GLCD()           _CONFIG_DRIVE_PORT_OUTPUT_VALUE(1, (GLCD_RST | GLCD_RW | GLCD_ENA | GLCD_RS), (GLCD_RW), (PORT_SRE_SLOW | PORT_DSE_MID)); _CONFIG_DRIVE_PORT_OUTPUT_VALUE(1, (GLCD_CS0 | GLCD_CS1), (0), (PORT_SRE_SLOW | PORT_DSE_MID)); _CONFIG_PORT_INPUT(1, DATA_LINES, (PORT_PS_UP_ENABLE))

    #define GLCD_DATAASOUTPUT()        _DRIVE_PORT_OUTPUT(1, 0xff)
    #define GLCD_DATAASINPUT()         _FLOAT_PORT(1, 0xff)

    #define GLCD_DATAOUT(data)         _WRITE_PORT_MASK(1, data, 0xff)
    #define GLCD_DATAIN()              _READ_PORT(1)
...
//
    ...
    // LEDs
    //
    #if defined EMCRAFT_K70F120M || defined EMCRAFT_K61F150M || defined K61FN1_50M
                                        // '0'            '1'     input state   center (x,   y)   0 = circle, radius, controlling port, controlling pin 
...
#define SW_UPLOAD_COMPLETED   "<html><head><meta http-equiv=""refresh"" content=""5;URL=0Menu.htm""><title>iMX RT SW Uploaded</title></head><body bgcolor=#ffffff text=#000000 topmargin=3 marginheight=3><center><td valign=top class=h><font color=#ff0000 style=font-size:30px><b style='mso-bidi-font-weight:normal'>&micro;Tasker</font> - SW Update</b></td><br></td><td align=left><br><br>SW Upload successful. The iMX RT target will now reset and start the new program. Please wait 5s...</body></html>"
