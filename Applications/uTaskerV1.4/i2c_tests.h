/**********************************************************************
    ...
    *********************************************************************
    ...
    06.02.2020 Correct TEST_FXOS8700 with READ_MAGNETOMETER configuration{5}
...
*/
  ...
  //#define DISPLAY_ACCELEROMETER_VALUES                                 // print values to debug output irrespective of debug setting
        ...
        #if defined MIMXRT1010 || defined MIMXRT1015 || defined MIMXRT1020 || defined MIMXRT1024 || defined MIMXRT1050 || defined MIMXRT1060 | defined MIMXRT1064 || defined FRDM_K22F || defined TWR_K22F120M
        ...
        #elif defined KINETIS_K80 || defined FRDM_KL82Z || defined TWR_KL82Z72M || defined FRDM_KE15Z
...
static void acc_data_ready(void)
{
    ...
    #endif
                iDisplayRate = 0;
    #if defined TEST_FXOS8700 && defined READ_MAGNETOMETER && !defined TEST_MMA8451Q
    ...
    case ACC_MAGNETOMETER:
        if (fnRead(I2CPortID, ucInputMessage, 6) != 0) {                 // if the status read has completed
            ...
            if (++iDisplayRate > 100) {
                ...
                iDisplayRate = 0;
                fnWrite(I2CPortID, (unsigned char *)ucSetAccelerometerRead, sizeof(ucSetAccelerometerRead)); // write the register address to read
                fnRead(I2CPortID, (unsigned char *)ucReadAccelerometerState, 0); // start the read process of the next accelerometer status
                iAccelerometerState = ACC_X_Y_Z;
                break;                                                   // {5} break to avoid already queuing next magnetometer read
            }
            fnWrite(I2CPortID, (unsigned char *)ucSetMagnetometerRead, sizeof(ucSetMagnetometerRead)); // write the register address to read
            fnRead(I2CPortID, (unsigned char *)ucReadMagnetometerState, 0); // start the read process of the next status
        }
