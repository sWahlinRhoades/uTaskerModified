/**********************************************************************
    ...
    ---------------------------------------------------------------------
    File:      spi_flash_AT25SF.h - adesto
    ...
    **********************************************************************/
    ...
    static void fnSPI_command_AT25SF(unsigned char ucCommand, unsigned long ulPageNumberOffset, int iChipSelect, volatile unsigned char *ucData, MAX_FILE_LENGTH DataLength);
    #else
    static void fnSPI_command_AT25SF(unsigned char ucCommand, unsigned long ulPageNumberOffset, volatile unsigned char *ucData, MAX_FILE_LENGTH DataLength);
    #endif
#endif
...
#define DEVICE_ID_1_DATA_ADESTO_FLASH_128M  0x01                         // 128MBit / 16MegByte - AT25SF128A
        ...
        static unsigned char fnCheckAT25SFXXX(int iChipSelect);
        static const STORAGE_AREA_ENTRY spi_flash_storage_AT25SF = {
        ...
        static unsigned char fnCheckAT25SFXXX(void);
        static const STORAGE_AREA_ENTRY spi_flash_storage_AT25SF = {
            (void *)&default_flash,                                      // link to internal flash
            (unsigned char *)(SPI_FLASH_START),                          // spi flash area start
            (unsigned char *)(SPI_FLASH_START + (SPI_DATA_FLASH_SIZE - 1)), // size of SPI flash area
    ...
    #if defined SPI_FLASH_MULTIPLE_CHIPS
    ucSPI_FLASH_Type[0] = fnCheckAT25SFXXX(0);                           // flag whether the first SPI FLASH device is connected
    #else
    ucSPI_FLASH_Type[0] = fnCheckAT25SFXXX();                            // flag whether the SPI FLASH device is connected
    #endif
    if (ucSPI_FLASH_Type[0] < AT25SF128A)
    {                                                                    // we expect at least this part to be available
        ...
        while (++i < SPI_FLASH_DEVICE_COUNT) {
            ucSPI_FLASH_Type[i] = fnCheckAT25SFXXX(i);
        }
    #endif
        UserStorageListPtr = (STORAGE_AREA_ENTRY *)&spi_flash_storage_AT25SF; // insert spi flash as storage medium
    #if defined SPI_FLASH_SECOND_SOURCE_MODE
        fnSPI_command = fnSPI_command_AT25SF;
    #endif
    }
#endif
...
// This is the main interface code to the adesto SPI FLASH device
//
/* =================================================================== */
/*                           ATIS25F driver                            */
/* =================================================================== */
    ...
    #endif
{
    ...
    if (SPI_FLASH_Danger[iChipSelect] != 0) {                            // check whether the chip is ready to work, if not wait
        ...
        do {
            fnSPI_command_AT25SF(READ_STATUS_REGISTER_1, 0, __EXTENDED_CS &ucStatus, 1); // read busy status register
...
#endif
    fnSPI_command_AT25SF(READ_JEDEC_ID, 0, __EXTENDED_CS ucID, sizeof(ucID)); // read the SPI flash identity
    if (ucID[0] == MANUFACTURER_ID_ADESTO) {                             // adesto memory part recognised
        switch (ucID[2]) {
        case DEVICE_ID_1_DATA_ADESTO_FLASH_128M:                         // 128MBit / 16MegByte - AT25SF128A
            ucReturnType = AT25SF128A;
            break;
