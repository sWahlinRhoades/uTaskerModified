...
#endif
{
    ...
    if (ucID[0] == MANUFACTURER_ID_ATMEL) {                              // ATMEL D-part recognised
        ...
        if (iRtn != NO_SPI_FLASH_AVAILABLE) {
            ...
            fnSPI_command(READ_STATUS_REGISTER_1, 0, __EXTENDED_CS ucID, 1); // read the status register
    ...
    fnSPI_command(READ_STATUS_REGISTER_1, 0, __EXTENDED_CS ucID, 1);     // it is possibly a B-device so check the status register
