/************************************************************************
    ...
    ---------------------------------------------------------------------
    File:        spi_sc16IS7xx.h [Atmel AT91SAM7X128 / 256 / 512]
...
static void fnSendExtSCI_byte(QUEUE_HANDLE Channel, unsigned char ucAddress, unsigned char ucData)
{
    ...
    while (!(SC16IS7XX_SPI_SR & SPI_RDRF)) {                             // wait until tx byte has been sent and rx byte has been completely received
#ifdef _WINDOWS
        SC16IS7XX_SPI_SR |= SPI_RDRF;
#endif
    };
...
static void ext_sci_1_interrupt(void)
{
    ...
    while (!(CHECK_UART_2_3_INT_PENDING())) {                            // while the interrupt input stays low
        ...
        if ((fnCheckExtinterrupts(2) == 0) && (fnCheckExtinterrupts(3) == 0)) {
            fnSimulateInputChange(EXT_UART_2_3_INT_PORT, fnMapPortBit(EXT_UART_2_3_INT_BIT), SET_INPUT); // remove interrupt if no more waiting
        }
    #endif
    }
}
...
extern void fnConfigExtSCI(QUEUE_HANDLE Channel, TTYTABLE *pars)
{
    ...
    interrupt_setup.int_port_sense = (IRQ_FALLING_EDGE | IRQ_PULLUP_ENABLE); // interrupt on falling edge
#if NUMBER_EXTERNAL_SERIAL > 2
