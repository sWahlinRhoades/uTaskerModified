...
extern int fnSimTimers(void)
{
    ...
    if (T0TCR & COUNTER_ENABLE) {                                        // ensure enabled
        ...
        case PCLK_TIMER0_CCLK_1:
            ulTickCount = (unsigned long)((float)((float)TICK_RESOLUTION*(float)PCLK)/(float)(1*1000000)); // clock pulses in tick period
            break;
        case PCLK_TIMER0_CCLK_2:
            ulTickCount = (unsigned long)((float)((float)TICK_RESOLUTION*(float)PCLK)/(float)(2*1000000)); // clock pulses in tick period
            break;
        case PCLK_TIMER0_CCLK_4:
            ulTickCount = (unsigned long)((float)((float)TICK_RESOLUTION*(float)PCLK)/(float)(4*1000000)); // clock pulses in tick period
            break;
        case PCLK_TIMER0_CCLK_8:
            ulTickCount = (unsigned long)((float)((float)TICK_RESOLUTION*(float)PCLK)/(float)(8*1000000)); // clock pulses in tick period
            break;
        }
        if (T0PR != 0) {                                                 // {16}
            ulTickCount /= (T0PR + 1);
            ...
            case WD_CLOCK_PCLK:
                switch (PCLKSEL0 & PCLK_WDT_CCLK_8) {
                    case PCLK_WDT_CCLK_1:
                        ulTickCount = ((PCLK/1000000)*TICK_RESOLUTION);
                        break;
                    case PCLK_WDT_CCLK_2:
                        ulTickCount = ((PCLK/2000000)*TICK_RESOLUTION);
                        break;
                    case PCLK_WDT_CCLK_4:
                        ulTickCount = ((PCLK/4000000)*TICK_RESOLUTION);
                        break;
                    case PCLK_WDT_CCLK_8:
                        ulTickCount = ((PCLK/8000000)*TICK_RESOLUTION);
                        break;
                }
                if (WDTV >= ulTickCount) {
                    WDTV -= ulTickCount;
    ...
    if (RTC_CCR & CCR_CLKEN) {                                           // if RTC is enabled
        if (RTC_CCR & CCR_CLKSRC_32K) {                                  // if clocked from 32kHz
            RTC_CTC += (32768/(1000000/TICK_RESOLUTION));                // the count per TICK based on 32kHz source
        }
        else {
            RTC_CTC += ((PCLK/(RTC_PREINT + 1))/(1000000/TICK_RESOLUTION)); // the count per TICK
        }
